...
class Bootstrap {
    ...
    function loadConfig() {
        ...
        elseif(file_exists(INCLUDE_DIR.'settings.php')) { //OLD config file.. v 1.6 RC5
            ...
            if(!strcasecmp(basename($_SERVER['SCRIPT_NAME']), 'settings.php'))
                Http::response(500,
                    'โปรดเปลี่ยนชื่อไฟล์ตั้งค่า include/settings.php เป็น include/ost-config.php ก่อนดำเนินการต่อ!');
        } elseif(file_exists(ROOT_DIR.'setup/'))
            Http::redirect(ROOT_PATH.'setup/');

        if(!$configfile || !file_exists($configfile))
            Http::response(500,'<b>เกิดข้อผิดพลาดในการโหลดไฟล์ตั้งค่า กรุณาติดต่อผู้ดูแลระบบ</b>');
    ...
    function connect() {
        ...
        if (!db_connect(DBHOST, DBUSER, DBPASS, $options)) {
            $ferror=sprintf('ไม่สามารถติดต่อฐานข้อมูล — %s',db_connect_error());
        }elseif(!db_select_database(DBNAME)) {
            $ferror=sprintf('ฐานข้อมูลผิดพลาดหรือไม่ถูกต้อง: %s',DBNAME);
        }
    ...
    function croak($message) {
        $msg = $message."\n\n".THISPAGE;
        Mailer::sendmail(ADMIN_EMAIL, 'iService Fatal Error', $msg,
            sprintf('"iService Alerts"<%s>', ADMIN_EMAIL));
