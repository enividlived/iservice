...
class OverviewReportAjaxAPI extends AjaxController {
    ...
    function getData() {
        ...
        $groups = array(
            ...
            "topic" => array(
                ...
                "fields" => "CONCAT_WS(' / ',"
                    ...
                    ."T1.topic) as name ",
                "headers" => array(__('คำขอใช้บริการ')),
                "filter" => '1'
            ),
            "staff" => array(
                "table" => STAFF_TABLE,
        ...
        return array("columns" => array_merge($info['headers'],
                        array(__('คำขอ'),__('มอบหมาย'),__('เลยกำหนด'),__('เสร็จสิ้น'),__('เปิดขึ้นใหม่'),
                              __('เวลาบริการ'),__('เวลาตอบกลับ'))),
                     "data" => $rows);
    }
    ...
    function downloadTabularData() {
        ...
        foreach ($data['data'] as $row)
            $csv .= "\n" . '"' . implode('","', $row) . '"';
        Http::download(
            sprintf('%s-report.csv', $this->get('group', __('แผนก'))),
            'text/csv', $csv);
    }
