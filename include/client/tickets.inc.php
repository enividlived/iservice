<?php
if(!defined('OSTCLIENTINC') || !is_object($thisclient) || !$thisclient->isValid()) die('ปฏิเสธการเข้าถึง');
...
$showing.=($status)?(' '.$results_type):' '.__('คำขอใช้บริการทั้งหมด');
if($search)
...
?>
<h1><?php echo __('คำขอใช้บริการ');?></h1>
<br>
<form action="tickets.php" method="get" id="ticketSearchForm">
    ...
    <select name="status">
        ...
        <option value="open"
            ...
            <?php echo _P('ticket-status', 'รับเรื่องแล้ว');?> (<?php echo $thisclient->getNumOpenTickets(); ?>)</option>
        <?php
        if($thisclient->getNumClosedTickets()) {
            ?>
        <option value="closed"
            <?php echo ($status=='closed') ? 'selected="selected"' : '';?>>
            <?php echo __('ดำเนินการแล้ว');?> (<?php echo $thisclient->getNumClosedTickets(); ?>)</option>
        <?php
        } ?>
    </select>
    <input type="submit" value="<?php echo __('ค้นหา');?>">
</form>
<a class="refresh" href="<?php echo Format::htmlchars($_SERVER['REQUEST_URI']); ?>"><?php echo __('เรียกใหม่'); ?></a>
<table id="ticketTable" width="800" border="0" cellspacing="0" cellpadding="0">
    <caption><?php echo $showing; ?></caption>
    <thead>
        <tr>
            <th width="100">
                <a href="tickets.php?sort=ID&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="เรียงตามหมายเลขคำขอใช้บริการ" > <h4 align="center"><?php echo __('เลขที่คำขอ');?></h4></a>
            </th>
            <th width="75" >
                <a href="tickets.php?sort=date&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="เรียงตามวันที่สร้าง"> <h4 align="center"><?php echo __('วันที่สร้าง');?></h4></a>
            </th>
            <th width="110">
                <a href="tickets.php?sort=status&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="เรียงตามสถานะ"> <h4 align="center"><?php echo __('สถานะ');?></h4></a>
            </th>
            <th nowrap>
                <a href="tickets.php?sort=subj&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="เรียงตามเรื่อง"> <h4 align="center"><?php echo __('เรื่อง');?></h4></a>
            </th>
            <th width="120">
                <a href="tickets.php?sort=dept&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="เรียงตามแผนก"> <h4 align="center"><?php echo __('แผนก');?></h4></a>
            </th>
        </tr>
    </thead>
    <tbody>
     ...
     if($res && ($num=db_num_rows($res))) {
        ...
        while ($row = db_fetch_array($res)) {
            ...
            <tr id="<?php echo $row['ticket_id']; ?>">
                ...
                <td>&nbsp;<?php if($row['status']=='ดำเนินการแล้ว'){ ?> <font color=007700> <?php echo $row['status'];?> <img src="..\iservice\images\Correct.png"> <?php }?></FONT>
                          <?php if($row['status']=='รับเรื่องแล้ว'){?> <font color=0000CC> <?php echo $row['status'];?> <img src="..\iservice\images\wait.png"> <?php }?></FONT> </td>
                <td>
        ...
        <?php
         ...
         echo '<tr><td colspan="6">'.__('ไม่พบผลการค้นหา').'</td></tr>';
     }
    ?>
    </tbody>
</table>
