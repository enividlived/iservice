...
if ($ticket->isClosed() && !$ticket->isReopenable())
    $warn = __('คำขอใช้บริการนี้ถูกตั้งสถานะเป็นดำเนินการแล้ว และไม่สามารถเปิดขึ้นใหม่ได้');
...
<div id="msg_info">
    ...
    <strong><?php echo __('ต้องการค้นหาคำขอใช้บริการอื่น?'); ?></strong></br>
    <a href="<?php echo ROOT_PATH; ?>login.php?e=<?php
        echo urlencode($thisclient->getEmail());
    ?>" style="text-decoration:underline"><?php echo __('เข้าสู่ระบบ'); ?></a>
    <?php echo sprintf(__('หรือ %s สมัครสมาชิก %s เพื่อเข้าใช้งาน'),
...
<table width="800" cellpadding="1" cellspacing="0" border="0" id="ticketInfo">
    <tr>
        <td colspan="2" width="100%">
            <h1>
                <?php echo sprintf(__('คำขอใช้บริการที่ %s'), $ticket->getNumber()); ?> &nbsp;
                <a href="tickets.php?id=<?php echo $ticket->getId(); ?>" title="รีเฟรช"><span class="Icon refresh">&nbsp;</span></a>
<?php if ($cfg->allowClientUpdates()
        // Only ticket owners can edit the ticket details (and other forms)
        && $thisclient->getId() == $ticket->getUserId()) { ?>
                <a class="action-button pull-right" href="tickets.php?a=edit&id=<?php
                     echo $ticket->getId(); ?>"><i class="icon-edit"></i> แก้ไข</a>
<?php } ?>
            </h1>
        </td>
    </tr>
...
<br>
<div class="subject"><?php echo __('เรื่อง'); ?>: <strong><?php echo Format::htmlchars($ticket->getSubject()); ?></strong></div>
<div id="ticketThread">
<?php
    ...
    <table border="0" cellspacing="0" cellpadding="3" style="width:100%">
        <tr>
            <td colspan="2">
                <?php
                if($ticket->isClosed()) {
                    $msg='<b>'.__('การตอบกลับคำขอใช้บริการนี้ จะเป็นการเปิดคำใช้บริการขึ้นใหม่!').'</b>';
                } else {
                    $msg=__('เพื่อความรวดเร็วในการบริการ กรุณากรอกข้อมูลให้ถูกต้องครบถ้วน');
                }
                ?>
...
?>
    ...
    <p style="padding-left:165px;">
        <input type="submit" value="<?php echo __('ตอบกลับ');?>">
        <input type="reset" value="<?php echo __('รีเซ็ต');?>">
        <input type="button" value="<?php echo __('ยกเลิก');?>" onClick="history.go(-1)">
    </p>
</form>
<?php
} ?>
