...
$info=array();
if ($list) {
    $title = __('อัพเดทรายการที่กำหนดเอง');
    $action = 'update';
    $submit_text = __('บันทึก');
    $info = $list->getInfo();
    $newcount=2;
} else {
    $title = __('เพิ่มรายการที่กำหนดเอง');
    $action = 'add';
    $submit_text = __('เพิ่มรายการ');
    $newcount=4;
}
...
<form action="" method="post" id="save">
    ...
    <h2><?php echo __('รายการที่กำหนดเอง'); ?>
    <?php echo $list ? $list->getName() : 'เพิ่มรายการ'; ?></h2>

<ul class="tabs">
    <li><a href="#definition" class="active">
        <i class="icon-plus"></i> <?php echo __('คำนิยาม'); ?></a></li>
    <li><a href="#items">
        <i class="icon-list"></i> <?php echo __('รายการ'); ?></a></li>
    <li><a href="#properties">
        <i class="icon-asterisk"></i> <?php echo __('คุณสมบัติ'); ?></a></li>
</ul>
...
<div id="definition" class="tab_content">
    ...
    <thead>
        <tr>
            <th colspan="2">
                ...
                <em><?php echo __(
                'รายการที่กำหนดเอง ใช้เพื่อสร้างรายการเฉพาะสำหรับใช้ในแบบฟอร์มที่กำหนดเอง'
    ...
    <tbody>
        <tr>
            <td width="180" class="required"><?php echo __('ชื่อ'); ?>:</td>
            <td>
                <?php
        ...
        <tr>
            <td width="180"><?php echo __('ชื่อจำนวนที่มากกว่าหนึ่ง'); ?>:</td>
            <td>
                <?php
                    if ($list && !$list->isEditable())
        ...
        <tr>
            <td width="180"><?php echo __('การเรียงลำดับ'); ?>:</td>
...
<div id="properties" class="tab_content" style="display:none">
    ...
    <thead>
        <tr>
            <th colspan="7">
                <em><strong><?php echo __('คุณสมบัติรายการ'); ?></strong>
                <?php echo __('คุณสมบัติของแต่ละรายการ'); ?></em>
            </th>
        </tr>
        <tr>
            <th nowrap></th>
            <th nowrap><?php echo __('ชื่อ'); ?></th>
            <th nowrap><?php echo __('ประเภท'); ?></th>
            <th nowrap><?php echo __('ตัวแปร'); ?></th>
            <th nowrap><?php echo __('ลบทิ้ง'); ?></th>
        </tr>
    </thead>
            ...
            <?php if ($f->isConfigurable()) { ?>
                <a class="action-button field-config"
                    ...
                    href="#form/field-config/<?php
                        echo $f->get('id'); ?>"><i
                        class="icon-cog"></i> <?php echo __('ตั้งค่า'); ?></a> <?php } ?></td>
            <td>
...
<div id="items" class="tab_content" style="display:none">
    ...
    <?php if ($list) {
        ...
        ?>
    <?php }
        else $showing = __('เพิ่มรายการย่อย');
    ?>
        <tr>
            <th colspan="5">
        ...
        <tr>
            <th></th>
            <th><?php echo __('ค่า'); ?></th>
            <?php
            if (!$list || $list->hasAbbrev()) { ?>
            <th><?php echo __(/* Short for 'abbreviation' */ 'คำย่อ'); ?> <em style="display:inline">&mdash;
                <?php echo __('คำย่อสำหรับรายการย่อย'); ?></em></th>
            <?php
            } ?>
            <th><?php echo __('ปิดใช้งาน'); ?></th>
...
<p class="centered">
    ...
    <input type="button" name="cancel" value="<?php echo __('ยกเลิก'); ?>"
