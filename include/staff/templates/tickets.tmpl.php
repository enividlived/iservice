...
<?php csrf_token(); ?>
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="2" width="940">
    ...
    foreach($results as $row) {
        ...
        $assigned='';
        if ($row['staff_id'])
            $assigned=sprintf('<span class="Icon staffAssigned">%s</span>',Format::truncate($row['staff'],100));
        elseif ($row['team_id'])
            $assigned=sprintf('<span class="Icon teamAssigned">%s</span>',Format::truncate($row['team'],100));
        else
            $assigned=' ';

        $status = ucfirst($row['status']);
        $tid=$row['number'];
        $subject = Format::htmlchars(Format::truncate($row['subject'],100));
        ...
        <tr id="<?php echo $row['ticket_id']; ?>">
            ...
            <?php
            if ($user) { ?>
            <td><?php echo Format::truncate($row['dept_name'], 100); ?></td>
