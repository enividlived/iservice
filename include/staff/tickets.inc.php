...
<?php
if(!defined('OSTSCPINC') || !$thisstaff || !@$thisstaff->isStaff()) die('ปฏิเสธการเข้าถึง');
...
if($search) {
  ...
  if( ($_REQUEST['query'] && strlen($_REQUEST['query'])<3)
      ...
      $errors['err']=__('คำค้นหาต้องมากกว่า 3 ตัวอักษร');
      $searchTerm='';
  }
}
...
switch(strtolower($_REQUEST['status'])){ //Status is overloaded
    case 'open':
        $status='open';
		$results_type=__('คำขอใช้บริการใหม่');
        break;
    case 'closed':
        $status='closed';
		$results_type=__('ดำเนินการแล้ว');
        $showassigned=true; //closed by.
        break;
    case 'overdue':
        $status='open';
        $showoverdue=true;
        $results_type=__('เลยกำหนด');
        break;
    case 'assigned':
        $status='open';
        $staffId=$thisstaff->getId();
        $results_type=__('คำขอใช้บริการของฉัน');
        break;
    case 'answered':
        $status='open';
        $showanswered=true;
        $results_type=__('กำลังดำเนินการ');
        break;
    default:
...
if($staffId && ($staffId==$thisstaff->getId())) { //My tickets
    $results_type=__('คำขอใช้บริการมอบหมายแล้ว');
...
if(!$results_type)
    $results_type = sprintf(__('%s คำขอใช้บริการ' /* %s will be a status such as 'open' */),
        mb_convert_case($status, MB_CASE_TITLE));

if($search)
    $results_type.= ' ('.__('ผลการค้นหา').')';
...
<div id='basic_search'>
    ...
    <table>
        <tr>
            ...
            <td><input type="submit" name="basic_search" class="button" value="<?php echo __('ค้นหา'); ?>"></td>
            <td>&nbsp;&nbsp;<a href="#" id="go-advanced">[<?php echo __('ขั้นสูง'); ?>]</a>&nbsp;<i class="help-tip icon-question-sign" href="#advanced"></i></td>
        </tr>
    </table>
    </form>
</div>
...
<div>
        <div class="pull-left flush-left">
            <h2><a href="<?php echo Format::htmlchars($_SERVER['REQUEST_URI']); ?>"
                title="<?php echo __('รีเฟรช'); ?>"><i class="icon-refresh"></i> <?php echo
        ...
        <div class="pull-right flush-right">
            ...
            class="icon-trash"></i> <?php echo __('ลบทิ้ง'); ?></a>
            <?php
            } ?>
            <?php
...
<?php csrf_token(); ?>
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="2" width="940">
    <thead>
        <tr>
	        ...
	        <th width="70">
                <a <?php echo $id_sort; ?> href="tickets.php?sort=ID&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                    title="<?php echo sprintf(__('เรียงตาม %s %s'), __('หมายเลขคำขอใช้บริการ'), __($negorder)); ?>"><?php echo __('ลำดับ'); ?></a></th>
	        <th width="70">
                <a  <?php echo $date_sort; ?> href="tickets.php?sort=date&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                    title="<?php echo sprintf(__('เรียงตาม %s %s'), __('วันที่'), __($negorder)); ?>"><?php echo __('วันที่'); ?></a></th>
	        <th width="280">
                 <a <?php echo $subj_sort; ?> href="tickets.php?sort=subj&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                    title="<?php echo sprintf(__('เรียงตาม %s %s'), __('เรื่อง'), __($negorder)); ?>"><?php echo __('เรื่อง'); ?></a></th>
            <th width="170">
                <a <?php echo $name_sort; ?> href="tickets.php?sort=name&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                     title="<?php echo sprintf(__('เรียงตาม %s %s'), __('ชื่อ'), __($negorder)); ?>"><?php echo __('จาก');?></a></th>
            <?php
            if($search && !$status) { ?>
                <th width="60">
                    <a <?php echo $status_sort; ?> href="tickets.php?sort=status&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                        title="<?php echo sprintf(__('เรียงตาม %s %s'), __('สถานะ'), __($negorder)); ?>"><?php echo __('สถานะ');?></a></th>
            <?php
            } else { ?>
                <th width="60" <?php echo $pri_sort;?>>
                    <a <?php echo $pri_sort; ?> href="tickets.php?sort=pri&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                        title="<?php echo sprintf(__('เรียงตาม %s %s'), __('ความสำคัญ'), __($negorder)); ?>"><?php echo __('ความสำคัญ');?></a></th>
            <?php
            }
                ...
                if(!strcasecmp($status,'closed')) { ?>
                    <th width="150">
                        <a <?php echo $staff_sort; ?> href="tickets.php?sort=staff&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                            title="<?php echo sprintf(__('เรียงตาม %s %s'), __("ผู้ปิดคำขอใช้บริการ"), __($negorder)); ?>"><?php echo __('ผู้ปิดคำขอใช้บริการ'); ?></a></th>
                <?php
                } else { //assigned to ?>
                    <th width="150">
                        <a <?php echo $assignee_sort; ?> href="tickets.php?sort=assignee&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                            title="<?php echo sprintf(__('เรียงตาม %s %s'), __('ผู้ได้รับมอบหมาย'), __($negorder)); ?>"><?php echo __('มอบหมาย'); ?></a></th>
                <?php
                }
            } else { ?>
                <th width="150">
                    <a <?php echo $dept_sort; ?> href="tickets.php?sort=dept&order=<?php echo $negorder;?><?php echo $qstr; ?>"
                        title="<?php echo sprintf(__('เรียงตาม %s %s'), __('แผนก'), __($negorder)); ?>"><?php echo __('แผนก');?></a></th>
            <?php
            } ?>
        </tr>
     </thead>
     <tbody>
        ...
        if($res && ($num=count($results))):
            ...
            $ferror=__('ไม่พบผลการค้นหาคำขอใช้บริการ');
        endif; ?>
    </tbody>
    <tfoot>
     <tr>
        <td colspan="7">
            ...
            <?php }else{
                echo '<i>';
                echo $ferror?Format::htmlchars($ferror):__('พบ 0 ผลการค้นหา');
                echo '</i>';
            } ?>
        </td>
     </tr>
    ...
    if($num>0){ //if we actually had any tickets returned.
        echo '<div>&nbsp;'.__('หน้า').':'.$pageNav->getPageLinks().'&nbsp;';
        echo '<a class="export-csv no-pjax" href="?a=export&h='
            .$hash.'&status='.$_REQUEST['status'] .'">'.__('นำออก').'</a>&nbsp;<i class="help-tip icon-question-sign" href="#export"></i></div>';
    } ?>
    </form>
</div>
...
<div style="display:none;" class="dialog" id="confirm-action">
    ...
    <p class="confirm-action" style="display:none;" id="mark_overdue-confirm">
        <?php echo __('คุณแน่ใจหรือว่าต้องการตั้งสถานะเป็น <font color="red"><b>เลยกำหนด</b></font>?');?>
    </p>
...
<div class="dialog" style="display:none;" id="advanced-search">
    <h3><?php echo __('ค้นหาคำขอใช้บริการขั้นสูง');?></h3>
    <a class="close" href=""><i class="icon-remove-circle"></i></a>
    <hr/>
    <form action="tickets.php" method="post" id="search" name="search">
        <input type="hidden" name="a" value="search">
        <fieldset class="query">
            <input type="input" id="query" name="query" size="20" placeholder="<?php echo __('คีย์เวิร์ด') . ' &mdash; ' . __('ถ้ามี'); ?>">
        </fieldset>
        <fieldset class="span6">
            <label for="statusId"><?php echo __('สถานะ');?>:</label>
            <select id="statusId" name="statusId">
                 <option value="">&mdash; <?php echo __('ทุกสถานะ');?> &mdash;</option>
                <?php
        ...
        <fieldset class="span6">
            <label for="deptId"><?php echo __('แผนก');?>:</label>
            <select id="deptId" name="deptId">
                <option value="">&mdash; <?php echo __('ทุกแผนก');?> &mdash;</option>
                <?php
        ...
        <fieldset class="span6">
            <label for="flag"><?php echo __('ป้าย');?>:</label>
            <select id="flag" name="flag">
                 <option value="">&mdash; <?php echo __('ทุกป้าย');?> &mdash;</option>
                 <?php
                 if (!$cfg->showAnsweredTickets()) { ?>
                 <option data-state="open" value="answered"><?php echo __('กำลังดำเนินการ');?></option>
                 <?php
                 } ?>
                 <option data-state="open" value="overdue"><?php echo __('เลยกำหนด');?></option>
            </select>
        </fieldset>
        <fieldset class="owner span6">
            <label for="assignee"><?php echo __('มอบหมายให้');?>:</label>
            <select id="assignee" name="assignee">
                <option value="">&mdash; <?php echo __('ทุกคน');?> &mdash;</option>
                <option value="0">&mdash; <?php echo __('ไม่ได้มอบหมาย');?> &mdash;</option>
                <option value="s<?php echo $thisstaff->getId(); ?>"><?php echo __('ฉัน');?></option>
                <?php
                ...
                if(($teams=Team::getTeams())) {
                    echo '<OPTGROUP label="'.__('ทีม').' ('.count($teams).')">';
        ...
        <fieldset class="span6">
            <label for="topicId"><?php echo __('บริการ');?>:</label>
            <select id="topicId" name="topicId">
                <option value="" selected >&mdash; <?php echo __('ทุกบริการ');?> &mdash;</option>
                <?php
        ...
        <fieldset class="owner span6">
            <label for="staffId"><?php echo __('ดำเนินงานโดย');?>:</label>
            <select id="staffId" name="staffId">
                <option value="0">&mdash; <?php echo __('ทุกคน');?> &mdash;</option>
                <option value="<?php echo $thisstaff->getId(); ?>"><?php echo __('ฉัน');?></option>
                <?php
                if(($users=Staff::getStaffMembers())) {
                    foreach($users as $id => $name)
                        echo sprintf('<option value="%d">%s</option>', $id, $name);
        ...
        <fieldset class="date_range">
            <label><?php echo __('ช่วงวัน').' &mdash; '.__('สร้าง');?>:</label>
            <input class="dp" type="input" size="20" name="startDate">
            <span class="between"><?php echo __('ถึง');?></span>
        ...
        <p>
            <span class="buttons pull-right">
                <input type="submit" value="<?php echo __('ค้นหา');?>">
            </span>
            <span class="buttons pull-left">
