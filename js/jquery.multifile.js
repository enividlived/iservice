        ...
        return this.each(function() {
            ...
            , addInput = function(event) {
                ...
                if(fObj.data('files')>=settings.max_uploads || (fObj.data('files')+file.count)>settings.max_uploads) {
                    alert('จำนวนไฟล์ที่เลือก ('+ settings.max_uploads+') ถึงลิมิตสูงสุดในการอัพโหลดต่อครั้ง');
                } else if(!$.fn.multifile.checkFileTypes(file, settings.allowedFileTypes)) {
                    var msg = 'ไม่อนุญาตรูปแบบไฟล์ที่เลือก';
                    if(file.count>1)
                        msg = 'ไม่อนุญาตรูปแบบไฟล์ที่เลือก';

                    alert('Error: '+msg);
                    $this.replaceWith(new_input);
                } else if(!$.fn.multifile.checkFileSize(file, settings.max_file_size)) {
                    var msg = 'ขนาดของไฟล์เกินกว่ากำหนด';
                    if(file.count>1)
                        msg = 'ขนาดของไฟล์เกินกว่ากำหนด';
            ...
            , bindRemoveInput = function($input, file) {
                ...
                return function(event) {
                    ...
                    if(confirm('คุณแน่ใจหรือว่าจะลบ '+file.name+'?')) {
