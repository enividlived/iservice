...
RedactorPlugins.draft = {
    init: function() {
        ...
        this.$draft_saved = $('<span>')
            ...
            .hide()
            .append($('<span>')
                .text(__('บันทึกแบบร่าง')));
        // Float the [Draft Saved] box with the toolbar
        this.$toolbar.append(this.$draft_saved);
        if (this.opts.draftDelete) {
            var trash = this.buttonAdd('deleteDraft', __('ลบแบบร่าง'), this.deleteDraft);
...
$(document).ajaxError(function(event, request, settings) {
    if (settings.url.indexOf('ajax.php/draft') != -1
        ...
        $('#overlay').show();
        alert(__('ไม่สามารถบันทึกแบบร่าง กรุณาเรียกหน้าปัจจุบันใหม่เพื่อเรียกคืนแบบร่างก่อนหน้า'));
        $('#overlay').hide();
    }
});
