...
if ($_POST && isset($_POST['luser'])) {
    if (!$_POST['luser'])
        $errors['err'] = __('กรุณากรอกชื่อเข้าสู่ระบบให้ถูกต้อง');
    elseif (($user = UserAuthenticationBackend::process($_POST['luser'],
        ...
        if ($user instanceof ClientCreateRequest) {
            if ($cfg && $cfg->isClientRegistrationEnabled()) {
                ...
                $errors['err'] = __('ปฏิเสธการเข้าถึง กรุณาติดต่อผู้ดูแลระบบเพื่อสร้างแอคเคาท์ให้กับคุณ');
        ...
        $errors['err'] = __('ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง กรุณาลองใหม่!');
    }
    $suggest_pwreset = true;
}
elseif ($_POST && isset($_POST['lticket'])) {
    if (!Validator::is_email($_POST['lemail']))
        $errors['err'] = __('กรุณากรอกอีเมลแอดเดรสและหมายเลขคำขอใช้บริการ');
    elseif (($user = UserAuthenticationBackend::process($_POST['lemail'],
        ...
        $user->sendAccessLink();
        $msg = sprintf(__("%s - ลิงค์เข้าสู่ระบบได้ถูกส่งแล้ว!"),
            Format::htmlchars($user->getName()->getFirst()));
        $_POST = null;
    } elseif(!$errors['err']) {
        $errors['err'] = __('อีเมลแอดเดรสหรือหมายเลขคำขอใช้บริการไม่ถูกต้อง กรุณาลองใหม่!');
    }
}
