        ...
        case 'sendmail':
            if (($acct=ClientAccount::lookupByUsername($_POST['userid']))) {
                if (!$acct->isPasswdResetEnabled()) {
                    $banner = __('แอคเคาท์ของคุณไม่สามารถรีเซ็ตรหัสผ่านได้ กรุณาติดต่อผู้ดูแลระบบ');
                }
                elseif ($acct->sendResetEmail()) {
                    $inc = 'pwreset.sent.php';
                }
                else
                    $banner = __('เกิดข้อผิดพลาด ไม่สามารถส่งอีเมลรีเซ็ตรหัสผ่านได้');
            }
            else
                $banner = sprintf(__('ไม่สามารถตรวจสอบชื่อผู้ใช้: %s'),
...
elseif ($_GET['token']) {
    $banner = __('กรอกชื่อผู้ใช้อีกครั้ง');
    $inc = 'pwreset.login.php';
...
elseif ($cfg->allowPasswordReset()) {
    $banner = __('กรอกชื่อผู้ใช้ด้านล่าง');
}
else {
    $_SESSION['_staff']['auth']['msg']=__('ระบบรีเซ็ตรหัสผ่าน ถูกปิดการใช้งาน');
