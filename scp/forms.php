...
if($_REQUEST['id'] && !($form=DynamicForm::lookup($_REQUEST['id'])))
    $errors['err']=sprintf(__('%s: ผิดพลาดหรือไม่ถูกต้อง'), __('ฟอร์มแบบกำหนดเอง'));
...
if($_POST) {
    ...
    switch(strtolower($_POST['do'])) {
        case 'update':
            foreach ($fields as $f)
                if (in_array($f, $required) && !$_POST[$f])
                    $errors[$f] = sprintf(__('%s จำเป็นต้องมี'),
            ...
            foreach ($form->getDynamicFields() as $field) {
                ...
                if (in_array($field->get('name'), $names))
                    $field->addError(__('ตัวแปรนี้มีชื่อที่ใช้อยู่แล้ว'), 'name');
                ...
                if ($form->get('type') == 'T' && $field->get('name') == 'subject') {
                    if (($f = $field->getField(false)->getImpl()) && !$f->hasData())
                        $field->addError(__('ต้องเป็นฟิลด์ที่ให้ผู้ใช้กรอกข้อความได้'),
                            'type');
                }
                if ($field->get('name'))
                ...
                if ($field->isValid())
                    ...
                    # notrans (not shown)
                    $errors["field-$id"] = __('ฟิลด์มีค่าที่ไม่ถูกต้อง');
        ...
        case 'add':
            ...
            foreach ($fields as $f) {
                if (in_array($f, $required) && !$_POST[$f])
                    $errors[$f] = sprintf('%s จำเป็นต้องมี',
                        mb_convert_case($f, MB_CASE_TITLE));
                elseif (isset($_POST[$f]))
                    $form->set($f, $_POST[$f]);
            }
        ...
        case 'mass_process':
            if(!$_POST['ids'] || !is_array($_POST['ids']) || !count($_POST['ids'])) {
                $errors['err'] = sprintf(__('คุณต้องเลือกอย่างน้อย %s'), __('หนึ่งแบบฟอร์มกำหนดเอง'));
            } else {
                    ...
                    case 'delete':
                        ...
                        if ($i && $i==$count)
                            $msg = sprintf(__('ลบ %s เรียบร้อย'),
                                _N('ฟอร์มแบบกำหนดเองที่เลือก', 'ฟอร์มแบบกำหนดเองที่เลือก', $count));
                        elseif ($i > 0)
                            $warn = sprintf(__('%1$d จาก %1$d %3$s ถูกลบ'), $i, $count,
                                _N('ฟอร์มแบบกำหนดเองที่เลือก', 'ฟอร์มแบบกำหนดเองที่เลือก', $count));
                        elseif (!$errors['err'])
    ...
    if ($errors)
        $errors['err'] = sprintf(__('ไม่สามารถเปลี่ยนแปลง %s ได้ กรุณาตรวจเช็คความถูกต้อง'), __('ฟอร์มแบบกำหนดเองนี้'));
    else
        $msg = sprintf(__('ปรับปรุง %s เรียบร้อย'),
            __('ฟอร์มแบบกำหนดเองนี้'));
}
