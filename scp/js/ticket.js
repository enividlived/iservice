...
var autoLock = {
    ...
    handleEvent: function(e) {
        ...
        if(!autoLock.lasteventTime) { //I hate nav away warnings..but
            $(document).on('pjax:beforeSend.changed', function(e) {
                return confirm(__("ข้อมูลที่คุณกรอกจะถูกทิ้งทั้งหมด!"));
            });
            $(window).bind('beforeunload', function(e) {
                return __("ข้อมูลที่คุณกรอกจะถูกทิ้งทั้งหมด!");
             });
        }
    ...
    onSubmit: function(e) {
        if(e.type=='submit') { //Submit. double check!
            ...
            if(autoLock.warn && !autoLock.lockId && autoLock.lasteventTime) {
                var answer=confirm(__('ไม่สามารถล็อคคำขอใช้บริการได้ อาจมีคนกำลังทำคำขอใช้บริการนี้อยู่ โปรดยืนยันหากคุณต้องการดำเนินการต่อ'));
                if(!answer) {
                    e.returnValue=false;
                    e.cancelBubble=true;
                    if(e.preventDefault) {
            ...
            case 'acquire':
                if(!lock.id) {
                    ...
                    if(warn && (!lock.retry || autoLock.lockAttempts>=autoLock.maxattempts)) {
                        ...
                        alert(__('ไม่สามารถล็อคคำขอใช้บริการได้ อาจมีผู้อื่นกำลังทำคำขอใช้บริการนี้อยู่'));
                    }
                }
                break;
        }
    ...
    discardWarning: function(e) {
        e.returnValue=__("ข้อมูลที่คุณกรอกจะถูกทิ้งทั้งหมด!");
    },
        ...
        if (info && !e.data('wrapped')) {
            ...
            e.wrap(caption).parent()
                .hover(
                    ...
                    .append('<a href="'+info.download_url+'" class="action-button pull-right no-pjax"><i class="icon-download-alt"></i> '+__('')+'</a>')
                );
            e.data('wrapped', true);
        }
    });
