...
if(!defined('INCLUDE_DIR')) die('เกิดปัญหา กรุณาติดต่อผู้ดูแลระบบ');
        ...
        case 'sendmail':
            if (($staff=Staff::lookup($_POST['userid']))) {
                if (!$staff->hasPassword()) {
                    $msg = __('ไม่สามารถรีเซ็ตรหัสผ่านได้ กรุณาติดต่อผู้ดูแลระบบ');
                }
                ...
                $msg = sprintf(__('ไม่สามารถตรวจสอบชื่อผู้ใช้ %s ได้'),
...
elseif ($cfg->allowPasswordReset()) {
    $msg = __('กรอกชื่อผู้ใช้ในช่องด้านล่าง');
}
else {
    $_SESSION['_staff']['auth']['msg']=__('ไม่สามารถรีเซ็ตรหัสผ่านได้');
